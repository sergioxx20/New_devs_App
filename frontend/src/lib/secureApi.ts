...
const getBackendUrl = () => {
  ...
  if (
    ...
    console.log(
      `[SecureAPI] Using relative URLs for ${window.location.hostname}`,
    );
...
export class SecureAPIClient {
  ...
  private interceptDirectQueries() {
    if (import.meta.env.DEV) {
      const ENFORCE =
        (import.meta.env as any).VITE_ENFORCE_SECURE_API === "true";
      ...
      supabase.from = (table: string) => {
        ...
        if (DEV_ALLOWLIST.has(table) || !ENFORCE) {
          ...
          console.warn(
            `⚠️ Legacy direct query allowed in DEV${ENFORCE ? " (allowlist)" : ""}:`,
            table,
          );
          if (!DEV_ALLOWLIST.has(table) && !ENFORCE) {
  ...
  private async getAuthHeaders(): Promise<HeadersInit> {
    ...
    if (!token) {
      console.log(
        "[SecureAPI] No cached token, waiting for session or validating...",
      );
  ...
  public setAccessToken(token: string | null) {
    ...
    if (this.cachedToken !== token) {
      ...
      if (previousTenant && token) {
        console.log(
          "[SecureAPI SECURITY] Tenant context changed - cache cleared",
          {
    ...
    try {
      ...
      if (token) {
        ...
        // Check if it's a valid JWT
          ...
          extractedTenantId =
            payload.user_metadata?.tenant_id || payload.tenant_id;
        }
        ...
        if (extractedTenantId) {
          ...
          if (this.isValidTenantId(extractedTenantId)) {
            ...
            console.error(
              "[SecureAPI] Invalid tenant ID format:",
              extractedTenantId,
            );
      ...
      console.error(
        "[SecureAPI] JWT parsing failed - clearing session:",
        error,
      );
  ...
  public clearEndpointCache(endpointPattern: string): number {
    ...
    if (cleared > 0) {
      console.log(
        `[SecureAPI] Cleared ${cleared} cache entries for pattern: ${endpointPattern}`,
      );
    }
      ...
      if (key.includes("secure/cleaning/reports")) {
        ...
        if (total === 0 && key.includes("overdue")) {
          suspiciousEntries.push({
            key,
            age,
            data: { total, itemsCount: items.length },
          });
        }
      }
    }
  ...
  public emergencySecurityClear(): void {
    console.warn(
      "[SecureAPI EMERGENCY] Security clear - all cache and session data cleared",
    );
    this.cachedToken = null;
    this.cachedTenantId = null;
    this.requestCache.clear();
  ...
  private async generateCacheKey(
    ...
    if (
      endpoint.includes("secure/cleaning/reports") ||
      endpoint.includes("properties")
    ) {
      console.log("[SecureAPI] Generated cache key:", {
        endpoint,
        cacheKey:
          cacheKey.length > 100 ? cacheKey.substring(0, 100) + "..." : cacheKey,
  ...
  private isCacheResultValid(
    ...
    if (
      ...
      (!("items" in cachedData) && !("data" in cachedData))
    ) {
  ...
  private async request<T>(
    ...
    if (!tenantId) {
      console.warn(
        `[API SECURITY] No valid tenant ID - bypassing cache for ${endpoint}`,
      );
    ...
    if (isGetRequest) {
      ...
      if (cached) {
        ...
        if (this.isCacheResultValid(endpoint, cached.data, cacheAge)) {
          console.log(
            `[API CACHE HIT] ${endpoint} (tenant: ${tenantId}) age: ${cacheAge}ms`,
          );
          return cached.data;
        } else {
          // Remove invalid cache entry
          console.log(
            `[API CACHE INVALID] ${endpoint} - removing stale/suspicious entry`,
          );
      ...
      if (pending) {
        console.log(
          `[API DEDUP] ${endpoint} - waiting for pending request (tenant: ${tenantId})`,
        );
        try {
          const result = await Promise.race([
            pending,
          ...
          console.log(
            `[API DEDUP SUCCESS] ${endpoint} - returned cached result`,
          );
          return result;
        } catch (error) {
          console.warn(
            "[SecureAPI] Pending request failed or timed out, making fresh request:",
            error,
          );
  ...
  private async executeRequest<T>(
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt === 1) {
          console.log(
            `🔒 Secure API Request: ${options.method || "GET"} ${endpoint}`,
          );
        } else {
          console.log(
            `🔒 Secure API Request: ${options.method || "GET"} ${endpoint} (retry ${attempt - 1}/${MAX_RETRIES - 1})`,
          );
        }
        ...
        if (!response.ok) {
          ...
          if (response.status === 403) {
            throw new TenantIsolationError(
              detail ||
                "Access denied: You can only access data from your organization",
            );
          }
          ...
          if (response.status === 401) {
            console.log(
              "[SecureAPI] Got 401, attempting to refresh session...",
            );

            // Import sessionValidator dynamically to avoid circular dependency
            const { sessionValidator } =
              await import("../utils/sessionValidator");
            ...
            if (refreshedSession?.access_token) {
              console.log(
                "[SecureAPI] Session refreshed, will retry with new token",
              );
              ...
              if (attempt < MAX_RETRIES) {
                throw new Error(
                  "Authentication refreshed, will retry with new token",
                );
              }
            }
          ...
          const msg =
            detail || bodyText || `${response.status} ${response.statusText}`;
        ...
        if (isGetRequest && data !== null && requestKey) {
          ...
          if (shouldCache) {
            ...
            console.log(
              `[API CACHE STORE] ${endpoint} (tenant: ${requestKey.split(":")[2]})`,
            );
          } else {
            console.log(
              `[API CACHE SKIP] ${endpoint} - result validation failed`,
            );
          }
        }
        ...
        console.error(
          `[SecureAPI] Attempt ${attempt}/${MAX_RETRIES} failed:`,
          error,
        );
        ...
        if (attempt < MAX_RETRIES) {
          const delay =
            RETRY_DELAY_BASE * Math.pow(2, attempt - 1) + Math.random() * 500;
    ...
    if (isGetRequest && requestKey) {
      ...
      this.requestCache.delete(requestKey);
      console.log(
        `[API CACHE CLEAR] ${endpoint} - cleared due to failed request`,
      );
    }
  ...
  async getReservations(filters?: {
    ...
    const filtered = list.filter((r: any) => {
      ...
      if (q) {
        ...
        if (!(gn.includes(q) || ge.includes(q) || rid.includes(q)))
          return false;
      }
      return true;
    });
   ...
   */
  async getReservationSuggestions(
    search: string,
    limit: number = 10,
    opts?: { status?: string },
  ) {
  ...
  async getProperties(filters?: {
    ...
    try {
      ...
      if (result && typeof result === "object") {
        console.log(
          "[SecureAPI.getProperties] Result keys:",
          Object.keys(result),
        );
        ...
        if ("items" in result) {
          console.log(
            "[SecureAPI.getProperties] Found items array with",
          ...
          console.log(
            "[SecureAPI.getProperties] Found data array with",
          ...
          console.log(
            "[SecureAPI.getProperties] Result is array with",
            result.length,
            "properties",
          );
          ...
          console.warn(
            "[SecureAPI.getProperties] Unexpected result format, returning empty data",
          );
          return { data: [], total: 0 };
        }
      }
      ...
      console.log(
        "[SecureAPI.getProperties] Returning empty data for null/undefined result",
      );
      return { data: [], total: 0 };
    } catch (error) {
   ...
   * Update an appliance
   */
  async updatePropertyAppliance(
    propertyId: string,
    applianceId: string,
    updates: any,
  ) {
    return this.request<any>(
      `/api/v1/properties/${propertyId}/appliances/${applianceId}`,
      {
        method: "PUT",
        body: JSON.stringify(updates),
      },
   ...
   * Update a contract
   */
  async updatePropertyContract(
    propertyId: string,
    contractId: string,
    updates: any,
  ) {
    return this.request<any>(
      `/api/v1/properties/${propertyId}/contracts/${contractId}`,
      {
        method: "PUT",
        body: JSON.stringify(updates),
      },
   ...
   */
  async getReputationProperties(filters?: { city?: string; status?: string }) {
    const params = new URLSearchParams();
    if (filters) {
      Object.entries(filters).forEach(([key, value]) => {
        if (value !== undefined) params.append(key, String(value));
  ...
  async getReputationNotes(propertyIds: string[]) {
    ...
    if (propertyIds && propertyIds.length)
      params.append("property_ids", propertyIds.join(","));
    return this.request<any>(`/api/v1/reputation/notes?${params}`);
  }

  async createReputationNote(payload: {
    property_id: string;
    content: string;
  }) {
  ...
  async getLockboxes(filters?: {
    ...
    if (filters?.lockbox_type)
      params.append("lockbox_type", filters.lockbox_type);
  ...
  async getDashboardSummary(
    ...
    return this.request<any>(
      `/api/v1/dashboard/monthlySummary?${queryParams}`,
      requestOptions,
    );
  }
  ...
  async updateMyDepartmentPreference(
    departmentId: string,
    showInSidebar: boolean,
  ) {
    return await this.request<any>(
      `/api/v1/departments/my-departments/${departmentId}/preference`,
      {
        method: "PUT",
  ...
  async getProcessDocuments(departmentId: string, status: string = "active") {
    const res = await this.request<any[]>(
      `/api/v1/departments/${departmentId}/process-documents?status=${status}`,
    );
    return Array.isArray(res) ? res : [];
  }

  async createProcessDocument(data: {
    title: string;
    content: any;
  ...
  async uploadProcessDocumentAttachment(documentId: string, file: File) {
    ...
    const res = await fetch(
      `${this.backendUrl}/api/v1/process-documents/${documentId}/attachments`,
      {
        method: "POST",
        headers,
        body: form,
  ...
  async uploadInlineAttachment(
    ...
    const res = await fetch(
      `${this.backendUrl}/api/v1/process-documents/${documentId}/attachments/inline`,
      {
        method: "POST",
        headers,
        body: form,
  ...
  async deleteProcessDocumentAttachment(attachmentId: string) {
    return this.request(
      `/api/v1/process-documents/attachments/${attachmentId}`,
      {
        method: "DELETE",
        headers: await this.getAuthHeaders(),
      },
  ...
  async getDepartmentDocuments(
    departmentId: string,
    status: string = "active",
  ) {
    const res = await this.request<any[]>(
      `/api/v1/departments/${departmentId}/documents?status=${status}`,
    );
    return Array.isArray(res) ? res : [];
  }
  ...
  async getPermissionTemplates(params?: {
    ...
    const res = await this.request<any>(
      `/api/v1/permission_templates?${queryParams}`,
    );
    return Array.isArray(res?.permission_templates)
      ? res.permission_templates
      : [];
  }
  ...
  async getCustomFieldValues(reservationId: string) {
    try {
      return await this.request<any>(
        `/api/v1/custom-fields/values/${reservationId}`,
      );
    } catch (error: any) {
   ...
   */
  async bulkUpdateCustomFieldValues(
  ...
  async getRecentUserLogs(limit: number = 10): Promise<any[]> {
    const res = await this.request<any>(
      `/api/v1/logs/recent?limit=${limit}&user_only=true`,
    );
  ...
  async checkPropertyExists(hostawayId: string): Promise<boolean> {
    try {
      const response = await this.request<any>(
        `/api/v1/properties/check-exists?hostaway_id=${hostawayId}`,
      );
  ...
  async getCleaningReports(filters: any, signal?: AbortSignal) {
    ...
    if (filters.booking_status)
      params.append("booking_status", String(filters.booking_status));
    if (filters.property_id)
      params.append("property_id", String(filters.property_id));
    ...
    try {
      ...
      if (result && typeof result === "object") {
        ...
        if (total === 0 && filters.date_from === "overdue") {
          console.warn(
            "[SecureAPI] Received empty overdue cleaning data - this may indicate a backend issue",
          );
        }

        // Specific validation for tomorrow's data
        const isTomorrowRequest =
          filters.date_from &&
          filters.date_to &&
        ...
        if (isTomorrowRequest) {
          console.log("[SecureAPI] Tomorrow cleaning request completed", {
            ...
            cacheKey: await this.generateCacheKey(
              "GET",
              endpoint,
              (await this.getTenantId()) || "unknown",
            ),
          });

          if (total === 0) {
            console.warn(
              "[SecureAPI] Tomorrow returned 0 cleanings - verify this is correct",
              {
                filters,
        ...
        console.log(
          `[SecureAPI] Cleaning request completed: ${endpoint.substring(0, 80)}... -> ${total} items`,
        );
      }
      ...
      console.error(
        `[SecureAPI] Cleaning request failed: ${endpoint.substring(0, 80)}...`,
        error,
      );
      throw error;
    }
  ...
  async reviewVerification(verificationId: string, reviewData: any) {
    return this.request<any>(
      `/api/v1/guest-portal/review-verification/${verificationId}`,
      {
        method: "POST",
  ...
  async translateEntity(
    ...
    return this.request<any>(
      `/api/v1/translations/${entityType}/${entityId}/translate`,
      {
        method: "POST",
        body: JSON.stringify({
          fields: fields,
...
export const secureReservations = {
  ...
  invalidateCache: (propertyId?: string) =>
    SecureAPI.invalidateReservationCache(propertyId),
  getCached: (forceRefresh?: boolean) =>
    SecureAPI.getCachedReservations(!!forceRefresh),
};
...
export const secureCleaning = {
  getReports: (filters: any, signal?: AbortSignal) =>
    SecureAPI.getCleaningReports(filters, signal),
...
export const secureFormulas = {
  ...
  create: (formula: {
    ...
    description?: string;
    composition: string;
  }) => SecureAPI.createFormula(formula),
  update: (
    id: string,
    updates: {
      name?: string;
...
if (import.meta.env.DEV) {
  console.warn(
    "🔒 SecureAPI initialized - Direct Supabase queries are now blocked",
  );

  // Make cache diagnostics available globally for debugging
  (window as any).secureApiDiagnostics = () => SecureAPI.getCacheDiagnostics();
  (window as any).clearCleaningCache = () =>
    SecureAPI.clearEndpointCache("secure/cleaning/reports");
  console.log(
    "🔧 Debug utilities added: secureApiDiagnostics(), clearCleaningCache()",
  );
}
